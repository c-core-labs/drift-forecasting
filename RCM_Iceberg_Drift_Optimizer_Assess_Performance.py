...
def rcm_iceberg_drift_optimizer_assess_performance(iceberg_lat0, iceberg_lon0, rcm_datetime0, iceberg_length0, next_rcm_time,
                                                   iceberg_lat_end, iceberg_lon_end, iceberg_length_end, wind_wave_data_path,
                                                   ocean_data_path_ssh_grad_time, ocean_data_path_u_curr, ocean_data_path_v_curr):
    deg_radius = 30
    g = 9.80665
    rho_water = 1023.6
    rho_air = 1.225
    ...
    drag_coeffs_bounds = ((0.01, 2.5), (0.01, 3.5))
    drag_coeffs_initial_guess = np.array([Ca, Cw])
    ...
    def iceberg_pos_error(drag_coeffs, Re, iceberg_lat_obs, iceberg_lon_obs, iceberg_times, iceberg_u0, iceberg_v0, iceberg_time_wind_wave_hours,
                    lat_wind, lon_wind, lat_curr, lon_curr, time_wind_wave_hours, u_wind, v_wind, u_curr, v_curr, lat_wave, lon_wave, lat_si, lon_si,
                    lat_ssh_grad_x, lon_ssh_grad_x, time_curr_ssh_hours, ssh_grad_x, lat_ssh_grad_y, lon_ssh_grad_y, ssh_grad_y,
                    iceberg_time_curr_ssh_hours, depth_curr, iceberg_times_dt, iceberg_sails, Hs, wave_dir_E, wave_dir_N,
                    iceberg_drafts, iceberg_lengths, iceberg_masses, omega, g, rho_air, rho_water, bathy_interp, am, siconc, sithick, usi, vsi, Csi, rho_si):
        Ca = drag_coeffs[0]
        Cw = drag_coeffs[1]
        ...
        for i in range(len(iceberg_times) - 1):
            ...
            grounded_status = iceberg_grounded_status[i]

            f_u_wind = RegularGridInterpolator((lat_wind, lon_wind, time_wind_wave_hours), u_wind, method='linear', bounds_error=False, fill_value=5.)
            u_wind_ib = float(f_u_wind([iceberg_lat, iceberg_lon, iceberg_time_wind_wave_hours[i]]))
            f_v_wind = RegularGridInterpolator((lat_wind, lon_wind, time_wind_wave_hours), v_wind, method='linear', bounds_error=False, fill_value=-5.)
            ...
            f_usi = RegularGridInterpolator((lat_si, lon_si, time_wind_wave_hours), usi, method='linear', bounds_error=True, fill_value=np.nan)
            usi_ib = float(f_usi([iceberg_lat, iceberg_lon, iceberg_time_wind_wave_hours[i]]))
            f_vsi = RegularGridInterpolator((lat_si, lon_si, time_wind_wave_hours), vsi, method='linear', bounds_error=True, fill_value=np.nan)
            vsi_ib = float(f_vsi([iceberg_lat, iceberg_lon, iceberg_time_wind_wave_hours[i]]))
            f_siconc = RegularGridInterpolator((lat_si, lon_si, time_wind_wave_hours), siconc, method='linear', bounds_error=True, fill_value=np.nan)
    ...
    if not isinstance(iceberg_length0, (int, float)) or iceberg_length0 <= 0:
        iceberg_length0 = 100.

    if not isinstance(iceberg_length_end, (int, float)) or iceberg_length_end <= 0:
        iceberg_length_end = 100.
    ...
    lat_si = wind_wave_data['lat_si']
    lon_si = wind_wave_data['lon_si']
    ...
    siconc = wind_wave_data['siconc']
    sithick = wind_wave_data['sithick']
    usi = wind_wave_data['usi']
    vsi = wind_wave_data['vsi']
    time_wind_wave_temp = []
    ...
    lat_si = lat_si.flatten()
    lon_si = lon_si.flatten()
    ...
    iceberg_lengths = np.interp(time_numeric, [time_start, time_end], [iceberg_length0, iceberg_length_end])
    iceberg_lat_obs = np.interp(time_numeric, [time_start, time_end], [iceberg_lat0, iceberg_lat_end])
    iceberg_lon_obs = np.interp(time_numeric, [time_start, time_end], [iceberg_lon0, iceberg_lon_end])
    ...
    result = minimize(fun=lambda drag_coeffs: iceberg_pos_error(drag_coeffs, Re, iceberg_lat_obs, iceberg_lon_obs,
                                                                         iceberg_times, iceberg_u0, iceberg_v0, iceberg_time_wind_wave_hours,
                                                                         lat_wind, lon_wind, lat_curr, lon_curr, time_wind_wave_hours, u_wind, v_wind,
                                                                         u_curr, v_curr, lat_wave, lon_wave, lat_si, lon_si,
                                                                         lat_ssh_grad_x, lon_ssh_grad_x, time_curr_ssh_hours, ssh_grad_x, lat_ssh_grad_y,
                                                                         lon_ssh_grad_y, ssh_grad_y, iceberg_time_curr_ssh_hours, depth_curr,
                                                                         iceberg_times_dt, iceberg_sails, Hs, wave_dir_E, wave_dir_N,
                                                                         iceberg_drafts, iceberg_lengths, iceberg_masses, omega, g, rho_air, rho_water,
                                                                         bathy_interp, am, siconc, sithick, usi, vsi, Csi, rho_si),
                      x0=drag_coeffs_initial_guess, method='L-BFGS-B', tol=1e-3, bounds=drag_coeffs_bounds, options={'eps': 0.01})
    Ca = result.x[0]
    Cw = result.x[1]
    drag_coeffs_final_array = np.array([Ca, Cw])
    iceberg_pos_error_final = iceberg_pos_error(drag_coeffs_final_array, Re, iceberg_lat_obs, iceberg_lon_obs,
                                                                         iceberg_times, iceberg_u0, iceberg_v0, iceberg_time_wind_wave_hours,
                                                                         lat_wind, lon_wind, lat_curr, lon_curr, time_wind_wave_hours, u_wind, v_wind,
                                                                         u_curr, v_curr, lat_wave, lon_wave, lat_si, lon_si,
                                                                         lat_ssh_grad_x, lon_ssh_grad_x, time_curr_ssh_hours, ssh_grad_x, lat_ssh_grad_y,
                                                                         ...
                                                                         iceberg_drafts, iceberg_lengths, iceberg_masses, omega, g, rho_air, rho_water,
                                                                         bathy_interp, am, siconc, sithick, usi, vsi, Csi, rho_si)
    iceberg_pos_error_final = iceberg_pos_error_final / 1000.
    return iceberg_pos_error_final, Ca, Cw, iceberg_u0, iceberg_v0
...
def assess_rcm_iceberg_drift_deterioration_forecaster(iceberg_lat0, iceberg_lon0, rcm_datetime0, iceberg_length, grounded_status, next_rcm_time,
                                                      ...
                                                      ocean_data_path_temp, ocean_data_path_sal, Ca, Cw, iceberg_u0, iceberg_v0):
    bathy_data_path = './GEBCO_Bathymetric_Data/gebco_2024.nc'
    hycom_lat_lon_depth_data_file = 'C:/Users/idturnbull/Documents/MATLAB/ExxonMobil_RCM_Project_Analysis/hycom_lat_lon_depth.mat'
    deg_radius = 30
    g = 9.80665
    ...
    return (iceberg_lat0, iceberg_lon0, iceberg_lats, iceberg_lons, iceberg_times, iceberg_length,
            iceberg_draft, iceberg_mass, rcm_datetime0, next_rcm_time, grounded_status, iceberg_lengths, iceberg_masses, iceberg_siconcs)
...
output_filepath = 'C:/Users/idturnbull/Documents/MATLAB/ExxonMobil_RCM_Project_Analysis/RCM_Iceberg_Optimized_Forecaster_Results/'
...
for p in range(len(rcm_shapefiles_names_2023)):
    ...
    gdf = shapefile.Reader(os.path.join(rcm_shapefiles_path_2023, rcm_shapefile))
    acqdates = []
    ...
    for m in range(len(sorted_indices) - 2):
        try:
            index0 = sorted_indices[m]
            index1 = sorted_indices[m + 1]
            point0 = gdf.shape(index0)
            iceberg_lat0 = point0.points[0][1]
            iceberg_lon0 = point0.points[0][0]
            ...
            next_rcm_time = gdf.record(index1)['acqDate']
            ...
            iceberg_pos_error_final, Ca, Cw, iceberg_u0, iceberg_v0 = rcm_iceberg_drift_optimizer_assess_performance(iceberg_lat0, iceberg_lon0,
                                                           rcm_datetime0, iceberg_length, next_rcm_time, iceberg_lat_end, iceberg_lon_end, iceberg_length_end,
                                                           era5_wind_wave_data_2023_file, hycom_data_2023_file_ssh_grad_time,
                                                           hycom_data_2023_file_u_curr, hycom_data_2023_file_v_curr)
            index0 = sorted_indices[m + 1]
            index1 = sorted_indices[m + 2]
            point0 = gdf.shape(index0)
            iceberg_lat0 = point0.points[0][1]
            iceberg_lon0 = point0.points[0][0]
            point1 = gdf.shape(index1)
            ...
            savemat(output_filepath + rcm_shapefile[:-4] + '_' + str(m + 2) + '.mat', {'iceberg_times': iceberg_times, 'iceberg_lat0': iceberg_lat0,
                                                                                                 ...
                                                                                                 'iceberg_masses': iceberg_masses, 'iceberg_siconcs': iceberg_siconcs,
                                                                                                 'iceberg_length_end': iceberg_length_end,
                                                                                                 'iceberg_pos_error_final': iceberg_pos_error_final, 'Ca': Ca, 'Cw': Cw})
        except:
            pass

for p in range(len(rcm_shapefiles_names_2024)):
    rcm_shapefile = rcm_shapefiles_names_2024[p]
    gdf = shapefile.Reader(os.path.join(rcm_shapefiles_path_2024, rcm_shapefile))
    acqdates = []
