...
def rcm_iceberg_drift_hindcaster(obs: Observation, t1: np.datetime64, si_toggle, metocean_folder_date):
    ...
    def iceberg_acc(iceberg_lat, iceberg_u, iceberg_v, iceberg_sail, iceberg_draft, iceberg_length, iceberg_mass, dt, am, omega, Cw, Ca, C_wave, g, rho_air, rho_water,
        ...
        u_curr[1] = -u_curr[1]
        v_curr[1] = -v_curr[1]
        ssh_grad_x = -ssh_grad_x
        ssh_grad_y = -ssh_grad_y
        usi = -usi
        vsi = -vsi
        wave_dir += 180.
    ...
    def duv_dt(t, uv):
        ...
        ib_acc_E, ib_acc_N = iceberg_acc(iceberg_lat, iceberg_u_init, iceberg_v_init, new_iceberg_sail, new_iceberg_draft,
                                         ...
                                         C_wave, g, rho_air, rho_water, u_wind_ib, v_wind_ib, [u_curr_ib, u_curr_ib2],
                                         [v_curr_ib, v_curr_ib2], ssh_grad_x_ib, ssh_grad_y_ib, Hs_ib, wave_dir_ib,
                                         siconc_ib, sithick_ib, usi_ib, vsi_ib, Csi, rho_si)
        return ib_acc_E, ib_acc_N

    def iceberg_det(iceberg_length, iceberg_mass, iceberg_lat, solar_rad, ice_albedo, Lf_ice, rho_ice, water_pot_temps,
                    water_sals, water_depths, air_temp,
        ...
        air_therm_diff_array = np.array(
            [[-100, 7.88e-6], [-50, 13.02e-6], [-30, 15.33e-6], [-20, 16.54e-6], [-10, 17.78e-6], [-5, 18.41e-6],
             [0, 19.06e-6], [5, 19.71e-6], [10, 20.36e-6], [20, 21.7e-6], [25, 22.39e-6], [30, 23.07e-6],
             [50, 25.91e-6],
             [75, 29.61e-6], [100, 33.49e-6], [125, 37.53e-6], [150, 41.73e-6], [175, 46.07e-6], [200, 50.56e-6],
             [250, 59.93e-6], [300, 69.79e-6]])
        air_kin_vis_array = np.array(
            [[-75, 7.4e-6], [-50, 9.22e-6], [-25, 11.18e-6], [-15, 12.01e-6], [-10, 12.43e-6], [-5, 12.85e-6],
        ...
        water_therm_diff_array = np.array(
            [[1.36, 1.37, 1.38, 1.38, 1.39, 1.4, 1.41, 1.41, 1.42, 1.42, 1.43, 1.43, 1.43],
        ...
        water_kin_vis_array = np.array(
            [[17.92, 18.06, 18.23, 18.43, 18.54, 18.65, 18.9, 19.16, 19.46, 19.77, 20.11, 20.46, 20.84, 21.24],
    ...
    previous_time = np.datetime64(previous_time)
    metocean_folder_date_str = metocean_folder_date
    metocean_folder_date = np.datetime64(metocean_folder_date)
    hindcast_time = rcm_datetime0
    # hindcast_time_date_str = np.datetime_as_string(hindcast_time, unit='D')

    dirname_wind_waves = np.datetime_as_string(metocean_folder_date, unit='D')
    d_wind_waves = metocean_folder_date_str.replace('-', '')
    dirname_airT_sw_rad = np.datetime_as_string(metocean_folder_date, unit='D')
    d_airT_sw_rad = metocean_folder_date_str.replace('-', '')
    dirname_ocean = np.datetime_as_string(metocean_folder_date, unit='D')
    d_ocean = metocean_folder_date_str.replace('-', '')
    ...
    iceberg_times = [hindcast_time]
    current_time = hindcast_time

    while current_time - np.timedelta64(1, 'h') > previous_time:
        current_time -= np.timedelta64(1, 'h')
        iceberg_times.append(current_time)

    if iceberg_times[-1] > previous_time:
        iceberg_times.append(previous_time)
    ...
    hindcast_times_wind_waves = []
    hindcast_times_ocean = []
    hindcast_times_airT_sw_rad = []
    ...
    hindcast_start_time_wind_waves = np.datetime64(metocean_folder_date_str + 'T' + hour_utc_str_wind_waves + ':00:00')
    hindcast_start_time_ocean = np.datetime64(metocean_folder_date_str + 'T' + hour_utc_str_ocean + ':00:00')
    hindcast_start_time_airT_sw_rad = np.datetime64(metocean_folder_date_str + 'T' + hour_utc_str_airT_sw_rad + ':00:00')

    time_count_wind_waves = hindcast_start_time_wind_waves
    time_count_ocean = hindcast_start_time_ocean
    time_count_airT_sw_rad = hindcast_start_time_airT_sw_rad

    while time_count_wind_waves <= rcm_datetime0 + np.timedelta64(1, 'h'):
        hindcast_times_wind_waves.append(time_count_wind_waves)
        time_count_wind_waves = time_count_wind_waves + np.timedelta64(1, 'h')

    while time_count_ocean <= rcm_datetime0 + np.timedelta64(1, 'h'):
        hindcast_times_ocean.append(time_count_ocean)
        time_count_ocean = time_count_ocean + np.timedelta64(1, 'h')

    while time_count_airT_sw_rad <= rcm_datetime0 + np.timedelta64(3, 'h'):
        hindcast_times_airT_sw_rad.append(time_count_airT_sw_rad)
        time_count_airT_sw_rad = time_count_airT_sw_rad + np.timedelta64(3, 'h')

    hindcast_times_wind_waves = np.array(hindcast_times_wind_waves, dtype='datetime64[s]')
    hindcast_times_ocean = np.array(hindcast_times_ocean, dtype='datetime64[s]')
    hindcast_times_airT_sw_rad = np.array(hindcast_times_airT_sw_rad, dtype='datetime64[s]')

    hindcast_times_wind_waves_hours = (hindcast_times_wind_waves.astype('datetime64[h]') - hindcast_start_time_wind_waves.astype('datetime64[h]')).astype(int)
    hindcast_times_ocean_hours = (hindcast_times_ocean.astype('datetime64[h]') - hindcast_start_time_ocean.astype('datetime64[h]')).astype(int)
    hindcast_times_airT_sw_rad_hours = (hindcast_times_airT_sw_rad.astype('datetime64[h]') - hindcast_start_time_airT_sw_rad.astype('datetime64[h]')).astype(int)

    fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + dirname_ocean + '/' + d_ocean + 'T' + hour_utc_str_ocean + \
            'Z_MSC_RIOPS_VOZOCRTX_DBS-all_PS5km_P' + str(hindcast_times_ocean_hours[0]).zfill(3) + '.nc'
    ...
    fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + dirname_ocean + '/' + d_ocean + 'T' + hour_utc_str_ocean + \
            'Z_MSC_RIOPS_SOSSHEIG_SFC_GRAD_PS5km_P' + str(hindcast_times_ocean_hours[0]).zfill(3) + '.nc'
    ...
    fname = (rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + dirname_wind_waves + '/' + d_wind_waves + 'T' +
             hour_utc_str_wind_waves + 'Z_MSC_GDWPS_UGRD_AGL-10m_LatLon0.25_PT' + str(hindcast_times_wind_waves_hours[0]).zfill(3) + 'H.nc')
    ...
    fname = (rootpath_to_metdata + 'GDPS_airT_sw_rad_forecast_files/' + dirname_airT_sw_rad + '/CMC_glb_TMP_TGL_2_latlon.15x.15_' +
                d_airT_sw_rad + hour_utc_str_airT_sw_rad + '_P' + str(hindcast_times_airT_sw_rad_hours[0]).zfill(3) + '.nc')
    airT_data = nc.Dataset(fname)
    lat_airT_sw_rad = airT_data.variables['latitude'][:]
    lon_airT_sw_rad = airT_data.variables['longitude'][:]
    airT_data.close()

    base_time_wind_waves = hindcast_times_wind_waves[0]
    time_increments_wind_waves = np.arange(hindcast_times_wind_waves_hours[0], hindcast_times_wind_waves_hours[-1], 1)
    ...
    base_time_ocean = hindcast_times_ocean[0]
    time_increments_ocean = np.arange(hindcast_times_ocean_hours[0], hindcast_times_ocean_hours[-1], 1)
    ...
    base_time_airT_sw_rad = hindcast_times_airT_sw_rad[0]
    time_increments_airT_sw_rad = np.arange(hindcast_times_airT_sw_rad_hours[0], hindcast_times_airT_sw_rad_hours[-1], 3)
    ...
    for i in range(len(iceberg_times) - 1):
        print('Hindcasting icebergs at ' + str(iceberg_times[i]) + ' UTC...')
        ...
        hindcast_time_wind_waves_before = hindcast_times_wind_waves[before_idx]
        hindcast_time_wind_waves_after = hindcast_times_wind_waves[after_idx]
        ...
        hindcast_time_airT_sw_rad_before = hindcast_times_airT_sw_rad[before_idx]
        hindcast_time_airT_sw_rad_after = hindcast_times_airT_sw_rad[after_idx]
        ...
        hindcast_time_ocean_before = hindcast_times_ocean[before_idx]
        hindcast_time_ocean_after = hindcast_times_ocean[after_idx]
        ...
        hindcast_time_ocean_before2 = hindcast_times_ocean[before_idx]
        hindcast_time_ocean_after2 = hindcast_times_ocean[after_idx]

        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + u_wind_file_before
        u_wind_data_before = nc.Dataset(fname)
        u_wind_before = np.squeeze(u_wind_data_before.variables['u10'][:])
        u_wind_data_before.close()

        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + u_wind_file_after
        u_wind_data_after = nc.Dataset(fname)
        u_wind_after = np.squeeze(u_wind_data_after.variables['u10'][:])
        u_wind_data_after.close()

        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + v_wind_file_before
        v_wind_data_before = nc.Dataset(fname)
        v_wind_before = np.squeeze(v_wind_data_before.variables['v10'][:])
        v_wind_data_before.close()

        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + v_wind_file_after
        v_wind_data_after = nc.Dataset(fname)
        v_wind_after = np.squeeze(v_wind_data_after.variables['v10'][:])
        v_wind_data_after.close()

        fname = rootpath_to_metdata + 'GDPS_airT_sw_rad_forecast_files/' + metocean_folder_date_str + '/' + airT_file_before
        airT_data_before = nc.Dataset(fname)
        airT_before = np.squeeze(airT_data_before.variables['t2m'][:]) - 273.15
        airT_data_before.close()

        fname = rootpath_to_metdata + 'GDPS_airT_sw_rad_forecast_files/' + metocean_folder_date_str + '/' + airT_file_after
        airT_data_after = nc.Dataset(fname)
        airT_after = np.squeeze(airT_data_after.variables['t2m'][:]) - 273.15
        airT_data_after.close()

        fname = rootpath_to_metdata + 'GDPS_airT_sw_rad_forecast_files/' + metocean_folder_date_str + '/' + solar_rad_file_before
        ...
        fname = rootpath_to_metdata + 'GDPS_airT_sw_rad_forecast_files/' + metocean_folder_date_str + '/' + solar_rad_file_after
        ...
        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + Hs_file_before
        Hs_data_before = nc.Dataset(fname)
        Hs_before = np.squeeze(Hs_data_before.variables['swh'][:])
        Hs_data_before.close()

        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + Hs_file_after
        Hs_data_after = nc.Dataset(fname)
        Hs_after = np.squeeze(Hs_data_after.variables['swh'][:])
        Hs_data_after.close()

        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + wave_dir_file_before
        ...
        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + wave_dir_file_after
        ...
        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + wave_pd_file_before
        wave_pd_data_before = nc.Dataset(fname)
        wave_pd_before = np.squeeze(wave_pd_data_before.variables['mp2'][:])
        wave_pd_data_before.close()

        fname = rootpath_to_metdata + 'GDWPS_wind_wave_forecast_files/' + metocean_folder_date_str + '/' + wave_pd_file_after
        wave_pd_data_after = nc.Dataset(fname)
        wave_pd_after = np.squeeze(wave_pd_data_after.variables['mp2'][:])
        wave_pd_data_after.close()

        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + u_curr_file_before
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + u_curr_file_after
        u_curr_data_after = nc.Dataset(fname)
        u_curr_after = np.squeeze(u_curr_data_after.variables['vozocrtx'][:])
        u_curr_data_after.close()

        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + v_curr_file_before
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + v_curr_file_after
        v_curr_data_after = nc.Dataset(fname)
        v_curr_after = np.squeeze(v_curr_data_after.variables['vomecrty'][:])
        v_curr_data_after.close()

        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + u_curr_file_before2
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + u_curr_file_after2
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + v_curr_file_before2
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + v_curr_file_after2
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + salinity_file_before
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + salinity_file_after
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + pot_temp_file_before
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + pot_temp_file_after
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + ssh_grad_file_before
        ...
        fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + ssh_grad_file_after
        ...
        if si_toggle:
            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + siconc_file_before
            ...
            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + siconc_file_after
            siconc_data_after = nc.Dataset(fname)
            siconc_after = np.squeeze(siconc_data_after.variables['iiceconc'][:])
            siconc_data_after.close()

            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + sithick_file_before
            ...
            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + sithick_file_after
            sithick_data_after = nc.Dataset(fname)
            sithick_after = np.squeeze(sithick_data_after.variables['iicevol'][:])
            sithick_data_after.close()

            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + usi_file_before
            usi_data_before = nc.Dataset(fname)
            usi_before = np.squeeze(usi_data_before.variables['itzocrtx'][:])
            usi_data_before.close()

            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + usi_file_after
            usi_data_after = nc.Dataset(fname)
            usi_after = np.squeeze(usi_data_after.variables['itzocrtx'][:])
            usi_data_after.close()

            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + vsi_file_before
            vsi_data_before = nc.Dataset(fname)
            vsi_before = np.squeeze(vsi_data_before.variables['itmecrty'][:])
            vsi_data_before.close()

            fname = rootpath_to_metdata + 'RIOPS_ocean_forecast_files/' + metocean_folder_date_str + '/' + vsi_file_after
        ...
        t1 = (hindcast_time_wind_waves_before - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        t2 = (hindcast_time_wind_waves_after - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        weight_wind_waves = (t_new - t1) / (t2 - t1)

        t1 = (hindcast_time_ocean_before - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        t2 = (hindcast_time_ocean_after - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        weight_ocean = (t_new - t1) / (t2 - t1)

        t1 = (hindcast_time_ocean_before2 - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        t2 = (hindcast_time_ocean_after2 - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        weight_ocean2 = (t_new - t1) / (t2 - t1)

        t1 = (hindcast_time_airT_sw_rad_before - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        t2 = (hindcast_time_airT_sw_rad_after - np.datetime64('1970-01-01T00:00:00Z')) / np.timedelta64(1, 's')
        weight_airT_sw_rad = (t_new - t1) / (t2 - t1)

        for k in range(len(iceberg_lats0)):
            print('Hindcasting iceberg ' + str(iceberg_ids[k]) + '...')
            ...
            if new_iceberg_mass > 0:
                solution = solve_ivp(duv_dt, (0., iceberg_times_dt[i]), [iceberg_u, iceberg_v], method='BDF',
                                     t_eval=[0., iceberg_times_dt[i]])
